...
num_init_cond = 5000 # We are avraging over x initial conditions
...
for N_val in N_vals
    ...
    for a_val in a_vals
        ...
        for trial_num in 1:num_trials
            ...
            # First we have to evolve spin_chain_A to a steady state.
            spin_chain_A = random_global_control_evolve(J_vec, spin_chain_A, a_val, Int(round(L^1.4)), J, S_NAUGHT)[end]
            
            results = [[[0.0 for _ in 1:global_L] for _ in 1:num_timesteps] for _ in 1:num_init_cond]
            for i in 1:num_init_cond
                ...
                if mid_pert
                    ...
                    # For preturbing whole chain
                    spin_chain_B = spin_chain_B + (epsilon/L) .* (make_random_state(L))
                end
            ...
            make_data_file(results_file_path * "N$(N_val)_a$(aval_path)_IC$(num_init_cond)_L$(L)_ep$(epsilon_val_name)_trial$(trial_num)_time_rand_OTOC_MidPert$(mid_pert).data", reduce(+, OTOC_A_B) ./ num_init_cond)
        end
    end
end
