...
@everywhere begin
    ...
    epsilon_prime_vals = [round(ep, digits=6) for ep in range(0.0001, a_val - 0.0001; length=100)]
...
# get all the data
for L_val in L_vals
    println("a_val: $(a_val) | L_val: $(L_val)")

    T_f = Int(round(L_val^z_val))

    for epsilon_prime in epsilon_prime_vals
        println("a_val: $(a_val) | L_val: $(L_val) | EpsilonPrime $(epsilon_prime)")
        ...
        let epsilon_prime=epsilon_prime, L_val=L_val, num_initial_conds=num_initial_conds
            all_init_outputs = @distributed (vcat) for init_cond in 1:num_initial_conds
                ...
                evolved_state = time_random_n_evolve_state(state, T_f, epsilon_prime, a_val)
        ...
        sample_filepath = "stavskya_mc/data/time_rand_n/rho_per_epsilon/IC1/L$(L_val)/a$(a_val_name)/IC$(num_initial_conds)_L$(L_val)_epsilon$(epsilon_val_name)_a$(a_val_name)_z$(z_val_name).csv"
