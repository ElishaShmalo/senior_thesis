...
@everywhere begin
    ...
    L_vals = [20000]
    epsilon_prime_c = 0.27365
    epsilon_prime_vals = [round(epsilon_prime_c + 0.0001 * i, digits = 6) for i in -2:2]
...
for L_val in L_vals
    ...
    for sig_val in sig_vals
        println("L_val: $(L_val) | sig: $(sig_val)")
        sig_val_name = replace("$sig_val", "." => "p")
        for epsilon_prime in epsilon_prime_vals
            println("L_val: $(L_val) | sig: $(sig_val) | EpsilonPrime $(epsilon_prime)")
            ...
            let sig_val=sig_val, epsilon_prime=epsilon_prime, L_val=L_val, num_initial_conds=num_initial_conds
                @sync begin
                    @distributed for init_cond in 1:num_initial_conds
                        ...
                        for t in 1:Int(round(T_f/time_step))
                            state = time_random_gauss_evolve_state(state, time_step, epsilon_prime, sig_val)
                            current_rho[t] = calculate_avg_alive(state)
                        end
                        
                        # Save init cond data as csv
                        sample_filepath = "stavskya_mc/data/time_rand_gauss/rho_per_time/IC1/L$(L_val)/sig$(sig_val_name)/IC1_L$(L_val)_epsilon$(epsilon_val_name)_timepref$(time_prefact)_timestep$(time_step)_sample$(init_cond+num_init_conds_offset).csv"
