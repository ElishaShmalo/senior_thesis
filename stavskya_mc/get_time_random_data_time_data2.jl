...
@everywhere begin
    ...
    L_vals = [32000]
    epsilon_prime_c = 0.26789
    epsilon_prime_vals = [round(epsilon_prime_c + 0.000001 * i, digits = 6) for i in -2:2 if round(epsilon_prime_c + 0.000001 * i, digits = 6)>= 0.267891]
            ...
            let delta_val_to_use=delta_val_to_use, epsilon_prime=epsilon_prime, L_val=L_val, num_initial_conds=num_initial_conds
                @sync begin
                    @distributed for init_cond in 1:num_initial_conds
                        ...
                        sample_filepath = "stavskya_mc/data/time_rand_delta/rho_per_time/IC1/L$(L_val)/delta$(delta_val_name)/IC1_L$(L_val)_epsilon$(epsilon_val_name)_timepref$(time_prefact)_timestep$(time_step)_sample$(init_cond+num_init_conds_offset).csv"
