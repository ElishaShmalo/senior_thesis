...
# for the upper/lower binary time-random Stavskaya model, with block disorder.
#
# 2026-09 successor of stavskya_mc/get_time_random_uppper_lower_binary.jl. The only
...
# stavskya_mc/data/block_rho_per_ep/time_rand_window_binary/... (absolute path, see naming.jl).
#
# Cluster:          cd stavskya_mc/block_disorder/submit && sbatch submit_upper_lower_rho_per_ep_block.sh
# Local smoke test: STAV_LOCAL_TEST=1 julia stavskya_mc/block_disorder/generators/get_upper_lower_binary_rho_per_ep_block.jl
...
@everywhere begin
    ...
    upper_start = 0.32
    lower_div = 20
    upper_epsilons = sort(union([round(i, digits=6) for i in upper_start:rate:0.341],
                                [round(i, digits=6) for i in 0.328:rate/2:0.33]))
