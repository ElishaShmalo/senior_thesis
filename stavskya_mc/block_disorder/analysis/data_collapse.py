"""DataCollapse helper, copied verbatim from cell 7 of
stavskya_mc/analyze_random_upper_lower_binary_rho_per_ep.ipynb (2026-09), so the new
block_disorder notebooks can import it instead of pasting it.

One change: `bootstrapping` used DataFrame.applymap, which was renamed DataFrame.map
in pandas 2.1 (applymap is deprecated). `_elementwise` uses whichever one exists.
Nothing else was changed.
"""


def _elementwise(df):
    return df.map if hasattr(df, "map") else df.applymap
...
def bootstrapping(df,params,p_,L_,p_range,nu,p_c,rng=0,Lmin=None,Lmax=None,size=None,replace=True,method='leastsq',p_c_vary=True,nu_range=(.5,2),**kwargs):
    ...
    df_resample=_elementwise(df_small)(lambda x: rng.choice(x,size=len(x) if size is None else min(size,len(x)),replace=replace))
