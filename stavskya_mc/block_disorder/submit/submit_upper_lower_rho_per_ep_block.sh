...
#SBATCH --job-name=ul_ep_block         # Assign a short name to your job
...
# Submit from this directory:  cd stavskya_mc/block_disorder/submit && sbatch submit_upper_lower_rho_per_ep_block.sh
# (same idea as the old stavskya_mc/submit_*.sh, which were submitted from stavskya_mc/).
# The generator writes to stavskya_mc/data/block_rho_per_ep/... no matter where it is launched from.

GEN=../generators/get_upper_lower_binary_rho_per_ep_block.jl
if [ ! -f "$GEN" ]; then
