...
@everywhere begin
    ...
    epsilon_prime_vals = [round(epsilon_prime_c + 0.000025 * i, digits = 6) for i in -2:2]
            ...
            let delta_val_to_use=delta_val_to_use, epsilon_prime=epsilon_prime, L_val=L_val, num_initial_conds=num_initial_conds
                @sync begin
                    @distributed for init_cond in 1:num_initial_conds
                        ...
                        sample_filepath = "stavskya_mc/data/time_rand_delta/rho_per_time/IC1/L$(L_val)/delta$(delta_val_name)/IC1_L$(L_val)_epsilon$(epsilon_val_name)_timepref$(time_prefact)_sample$(init_cond+num_init_conds_offset).csv"
