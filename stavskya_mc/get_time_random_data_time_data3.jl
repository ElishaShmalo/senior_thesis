...
@everywhere begin
    ...
    L_vals = [8000, 16000]
    L_vals = [1500, 24000]
    epsilon_prime_c = 0.28775
    epsilon_prime_vals = [round(epsilon_prime_c + 0.00001 * i, digits = 6) for i in 1:1]
    ...
    delta_vals = [0.1]
    [round(0.252 + 0.0001 * i, digits=6) for i in -20:20]
    time_step = 150000
end

rate = 0.000 
epsilon_c = 0.25375
start_epsilon = epsilon_c -2* rate
end_epsilon = epsilon_c +2* rate
            ...
            let delta_val_to_use=delta_val_to_use, epsilon_prime=epsilon_prime, L_val=L_val, num_initial_conds=num_initial_conds
                @sync begin
                    @distributed for init_cond in 1:num_initial_conds
                        ...
                        sample_filepath = "stavskya_mc/data/time_rand_delta/rho_per_time/IC1/L$(L_val)/delta$(delta_val_name)/epsilon$(epsilon_val_name)/IC1_L$(L_val)_epsilon$(epsilon_val_name)_timepref$(time_prefact)_timestep$(time_step)_sample$(init_cond+num_init_conds_offset).csv"
