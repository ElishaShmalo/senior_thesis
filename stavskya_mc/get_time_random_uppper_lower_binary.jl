...
@everywhere begin
    ...
    upper_epsilons = sort(union([round(i, digits=6) for i in upper_start:rate:0.55], [round(i, digits=6) for i in 0.35:0.01:0.45]))
    lower_epsilons = [round(i/10, digits=6) for i in upper_epsilons]
    p_vals = [1/2 for _ in 1:length(lower_epsilons)]
...
for L_val in L_vals
    ...
    for (upper_ep, lower_ep, p_val) in zip(upper_epsilons, lower_epsilons, p_vals)
        ...
        sample_filepath = "stavskya_mc/data/time_rand_window_binary/rho_per_epsilon/IC1/L$(L_val)/IC$(num_initial_conds)_L$(L_val)_epsilonu$(upper_epsilon_val_name)_epsilonl$(lower_epsilon_val_name)_pval$(p_val_name)_z$(z_val_name).csv"
